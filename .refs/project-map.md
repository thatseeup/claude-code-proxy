  ...
  proxy/                      — Go 백엔드 (module: github.com/seifghazi/claude-code-monitor)
    ...
    internal/
      ...
      service/
        ...
        pricing.go            — USD/Million 가격표 + `CalculateCostUSD(modelID, *AnthropicUsage) (float64, bool)` 순수 함수. 지원 base 모델: claude-fable-5, claude-opus-4-8, claude-opus-4-7, claude-opus-4-6, claude-sonnet-4-6, claude-haiku-4-5. lookup 직전에 `normalizeModelID` 가 트레일링 `-YYYYMMDD`(정확히 8자리 숫자) dated alias 만 잘라 base id 로 매핑(예: `claude-haiku-4-5-20251001` → `claude-haiku-4-5`). 그 외 prefix 매칭은 금지(`claude-opus-4-7-beta` 같은 임의 suffix 는 미매칭). cache_creation 객체가 있으면 ephemeral_5m / ephemeral_1h 각각 단가 적용; 없고 `CacheCreationInputTokens>0` 이면 전량 1h 단가. 미매칭/누락 → `ok=false`
